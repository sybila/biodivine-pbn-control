...
use crate::control::_algo_temporary_control::{reach_fwd_with_saturation, reach_bwd_with_saturation};
...
pub fn tgr_reduction(
    ...
    for var in graph.as_network().variables() {
        ...
        let reach_from_post = reach_fwd_with_saturation(graph, var_can_post.clone()).intersect(&universe);

        // Remove basin of the reachable area.
        if reach_from_post != universe {
            let reach_basin = reach_bwd_with_saturation(graph, reach_from_post.clone()).intersect(&universe)
        ...
        let post_extended_component =
            reach_bwd_with_saturation(graph, var_can_post.clone()).intersect(&reach_from_post);
        let bottom_region = reach_from_post.minus(&post_extended_component);

        // Remove basin of the bottom area.
        if !bottom_region.is_empty() {
            let bottom_basin = reach_bwd_with_saturation(graph, bottom_region.clone()).intersect(&universe)
                .minus(&bottom_region);
