...
impl ControlMap for AttractorControlMap {
    ...
    fn working_perturbations(
        ...
        let var_names = self.clone().perturbation_variables.into_iter().map(|v| {
            self.context
                .as_original()
                .as_network()
                .unwrap()
        ...
        for combination in var_names.powerset() {
            if max_robustness >= min_robustness
                && combination.len() > best_control_size
                && !return_all
            {
                return perturbations;
            }
            for true_values in combination.clone().iter().powerset() {
                let perturbation = HashMap::from_iter(
                    combination
                        .clone()
                        .into_iter()
                        .map(|var| (var.clone(), true_values.contains(&&var))),
                );
    ...
    fn perturbation_working_colors(&self, perturbation: &HashMap<String, bool>) -> GraphColors {
        ...
        for v in self.context.variables() {
            ...
            if perturbation.contains_key(var_name) {
                perturbed.require_perturbation(v, perturbation.get(var_name));
            } else {
                perturbed.exclude_perturbation(v, None);
            }
        }
    ...
    fn controllable_colors(&self) -> GraphColors {
        ...
        self.context
            ...
            .transfer_colors_from(
                &self.context.as_perturbed().empty_colors().copy(bdd),
                self.context.as_perturbed(),
            )
            .unwrap()
    }
