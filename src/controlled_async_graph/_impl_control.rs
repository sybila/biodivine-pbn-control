use crate::controlled_async_graph::{Bwd, ControlVariable, ControlledAsyncGraph};
...
use biodivine_lib_param_bn::async_graph::{AsyncGraph, AsyncGraphEdgeParams, DefaultEdgeParams};
...
impl ControlledAsyncGraph {
    ...
    pub fn new(network: BooleanNetwork) -> ControlledAsyncGraph {
        ...
        let edges =
            DefaultEdgeParams::new_with_custom_encoder(bn.clone(), encoder.clone()).unwrap();
        ...
        for v in self.network.variables() {
            ...
            if v == variable {
                let i_c = self
                    .encoder
                    .bdd_variables
                    .mk_not_var(controlled_var.is_controlled_variable);
                let c_v = self
                    .encoder
                    .bdd_variables
                    .mk_not_var(controlled_var.control_value_variable);
                let not_controlled = i_c.and(&c_v);
                allowed_control_params =
                    allowed_control_params.intersect(&BddParams::from(not_controlled));
            } else {
                let i_c1 = self
                    .encoder
                    .bdd_variables
                ...
                let i_c2 = self
                    .encoder
                    .bdd_variables
                    .mk_var(controlled_var.is_controlled_variable);
                let c_v2;
                if state.get_bit(v.into()) {
                    c_v2 = self
                        .encoder
                        .bdd_variables
                        .mk_var(controlled_var.control_value_variable);
                } else {
                    c_v2 = self
                        .encoder
                        .bdd_variables
                ...
                allowed_control_params =
                    allowed_control_params.intersect(&BddParams::from(viable_params));
            }
        }
    ...
    pub fn find_temporary_control(
        ...
        let strong_basin = find_strong_basin(
            self,
            target,
            &self
                .unit_params()
                .intersect(&BddParams::from(without_control)),
        );

        let strong_basin_seed = &StateSet::new_with_fun(self.graph.num_states(), |s| {
            if strong_basin.contains_key(&s) {
                Some(BddParams::from(self.encoder.bdd_variables.mk_true()))
            } else {
                None
            }
        });
