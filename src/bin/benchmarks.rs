...
fn main() {
    ...
    if !measure_basin && !measure_temp && !measure_perm {
        println!(
            "No measurements to be done, please provide at least one of: --{}, --{}, --{}",
            STRONG_BASIN_ARG, TEMP_CONTROL_ARG, PERM_CONTROL_ARG
        )
    }
    ...
    let config_file = matches
        .value_of(CONFIG_ARG)
        .unwrap_or("benchmark_config.json");
    let out_file = matches
        .value_of(OUT_FILE_ARG)
        .unwrap_or("benchmark_results.csv");
    ...
    for (model_name, attractors) in parsed_json.as_object().unwrap() {
        ...
        for a in attractors.as_array().unwrap() {
            ...
            let target_seed = &StateSet::new_with_fun(graph.num_states(), |s| {
                if s.eq(&target) {
            ...
            println!(
                "Strong basin computation time (ms): {:?}",
            ...
            for b in attractors.as_array().unwrap() {
                ...
                if measure_temp {
                    ...
                    println!(
                        "Temporary control computation time (ms): {:?}",
                        begin.elapsed().as_millis()
                    );
                ...
                if measure_perm {
                    ...
                    println!(
                        "Permanent control computation time (ms): {:?}",
                        begin.elapsed().as_millis()
                    );
...
fn create_csv(file_path: &str) {
    if !Path::new(file_path).exists() {
        ...
        if let Err(e) = writeln!(
            file,
            "time,method,duration,model,source,target,model_size,parameters_size,sb_size,tag"
        ) {
