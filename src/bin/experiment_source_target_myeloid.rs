...
fn main() {
    ...
    let attractors =
        biodivine_pbn_control::aeon::attractors::compute(perturbation_graph.as_original(), false);
    ...
    let monocyte = perturbation_graph
        .mk_unit_colored_vertices()
        .vertices()
        .fix_network_variable(cjun, true);
    let monocyte_att = vertices
        .iter()
        .find(|a| a.intersect(&monocyte).approx_cardinality() > 0.0)
        .unwrap();

    let eklf = resolve_var_id(&graph, "EKLF").unwrap();
    let erythrocyte = perturbation_graph
        .mk_unit_colored_vertices()
        .vertices()
        .fix_network_variable(eklf, true);
    let erythrocyte_att = vertices
        .iter()
        .find(|a| a.intersect(&erythrocyte).approx_cardinality() > 0.0)
        .unwrap();

    let source = monocyte_att.clone().into_iter().next().unwrap();
    let target = erythrocyte_att.clone().into_iter().next().unwrap();
    let res = perturbation_graph.permanent_control(
        &source,
        &target,
        perturbation_graph.unit_colors(),
        true,
    );
    ...
    println!(
        "All control {:?}",
        res.working_perturbations(0.99, true, false)
    );
    ...
    println!(
        "Empty control {:?}",
        res.perturbation_working_colors(&HashMap::new())
            .approx_cardinality()
    );
    println!(
        "Single control {:?}",
        res.perturbation_working_colors(&HashMap::from_iter(
            vec![
                ("Fli1".to_string(), false),
                ("PU1".to_string(), false),
                ("GATA1".to_string(), true)
            ]
            .into_iter()
        ))
        .approx_cardinality()
