...
/*
        // This is a trap set of all state-color-perturbation combinations that admit
        // the existence of a non-phenotype trap (and by extension a non-phenotype attractor).
        let mut trap = graph.unit_colored_vertices()
            .intersect_colors(&admissible_perturbations)
            .intersect_vertices(&not_phenotype);
            ...
            for var in graph.variables().rev() {
                ...
                if !can_leave.is_empty() {
                    ...
                    if trap.symbolic_size() > 100_000 {
                        println!("[{}] >> Trap progress: {} / {}", perturbation_size, trap.symbolic_size(), trap.approx_cardinality());
                    }
                    continue 'trap;
                }
            }
        ...
        println!("[{}] >> States in non-phenotype attractors: {} / {}", perturbation_size, trap.vertices().approx_cardinality(), graph.unit_colored_vertices().vertices().approx_cardinality());

        // Perform existential projection on state variables in cases where the variable is
        // not perturbed. In such cases, we don't care about the value.
...
*/
        ...
        /*let mut weak_basin = trap.clone();

        'bwd: loop {
            for var in graph.variables().rev() {
                let pre = graph.as_perturbed().var_pre(var, &weak_basin);
                if !pre.is_subset(&weak_basin) {
                    weak_basin = weak_basin.union(&pre);
                    if weak_basin.symbolic_size() > 100_000 {
                        println!("[{}] >> Weak basin progress: {} / {}", perturbation_size, trap.symbolic_size(), trap.approx_cardinality());
                    }
                    continue 'bwd;
                }
            }
        ...
        for var in graph.variables() {
            let mut can_leave = graph.as_perturbed()
                .var_can_post_out(var, &weak_basin)
                .into_bdd();
            ...
            for var in graph.variables() {
                ...
                if let Some(perturbation_var) = perturbation_var_map.get(&var) {
                    ...
                    let is_perturbed = can_leave
                        .var_select(*perturbation_var, true);
                    let is_not_perturbed = can_leave
                        .var_select(*perturbation_var, false)
                        .var_project(state_var);
                    can_leave = is_perturbed.or(&is_not_perturbed);
                } else {
                    // If the variable cannot be perturbed, we can eliminate it everywhere.
                    can_leave = can_leave.var_project(state_var);
                }
            }

            weak_basin = weak_basin.minus(&graph.empty_colored_vertices().copy(can_leave));
        }

        let control_map = weak_basin;*/
